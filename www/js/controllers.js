...
premiereApp.controller('MovieAppCtrl', function ($scope, $location, $http, $ionicLoading, $cordovaNetwork, $ionicPopup, $rootScope, $ionicModal, $ionicScrollDelegate) {
    ...
    $scope.loadMovies = function () {
        $http.get('https://yts.am/api/v2/list_movies.json?limit=40&sort_by=year')
            .then(function (res) {
    ...
    $scope.loadMore = function () {
        ...
        $http.get("https://yts.am/api/v2/list_movies.json?limit=40&sort_by=year&page=" + $scope.nextPage + "&genre=" + $scope.headeTitle)
            .then(function (more) {
    ...
    $scope.selectCategory = function (category) {
        ...
        $('meta[name=description]').attr('content', 'Find torrents online for '+category+" movies");
        $scope.closeCatModal(); // Hide the Category modal
        $ionicScrollDelegate.scrollTop(); // Automatically scroll to the top of the view
        $ionicLoading.show({
            template: '<p>Loading...</p><ion-spinner></ion-spinner>'
        });
        $http.get('https://yts.am/api/v2/list_movies.json?limit=40&sort_by=year&page=1&genre=' + category)
            .then(function (res) {
                $scope.movies = res.data.data.movies; // Import movies object from returned data
                $ionicLoading.hide(); // Hide loading animation when data is loaded
                $scope.loader = "show";
    ...
    $scope.searchMovie = function () {
        ...
        $scope.headeTitle = $scope.searchData.value; // movies
        $('meta[name=description]').attr('content', 'Find torrents online for '+ $scope.searchData.value);
        $ionicLoading.show({
            template: '<p>Loading...</p><ion-spinner></ion-spinner>'
        });

        //Get Search results
        $http.get('https://yts.am/api/v2/list_movies.json?limit=40&sort_by=year$&query_term=' + $scope.searchData.value)
            .then(function (res) {
...
premiereApp.controller('MovieDetails', function ($scope, $stateParams, $http, $window, $ionicLoading, $ionicModal, loadMovie, GetSubs, GetSuggestions) {
    ...
    $scope.movie = loadMovie.data.data.movie;
     ...
     $('meta[name=description]').attr('content', 'Downlod '+  $scope.movie.title+' torrent online for free');
    ...
    // yts.am/api for getting movies is open so it has no problem with CORS
...
premiereApp.controller('WatchList', function ($scope, $ionicLoading) {
    ...
    $('meta[name=description]').attr('content', 'Bookmark torrents of movies for later watching');
